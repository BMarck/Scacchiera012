...
public class ScacchieraController {
	...
	/**
	 * Classe interna che contiene il risultato riportato dal task: la
	 * combinazione di due interi, il numero di soluzioni ed il tempo impiegato
	 * 
	 * @author Fulvio
	 *
	 */
	...
	/**
	 * {@link Task} usato per il richiamo asincrono del metodo ricorsivo di
	 * calcolo delle soluzioni.
	 * 
	 * @author Fulvio
	 *
	 */
	public class TaskRicerca extends Task<ScacchieraController.Risultato> {

		private int size;

		public TaskRicerca(int size) {
			this.size = size;
		}
		...
		protected Risultato call() throws Exception {
			...
			Ricerca ricerca = new Ricerca(this);
			updateProgress(-1, -1);
		...
		/**
		 * Helper per poter chiamare updateProgress dalla funzione ricorsiva.
		 * 
		 * @param a
		 * @param b
		 */
		public void myUpdateProgress(double a, double b) {
	...
	void doCalcola(ActionEvent event) {
		...
		Task<Risultato> task = new TaskRicerca(size);
		...
		task.setOnSucceeded(new EventHandler<WorkerStateEvent>() {
			...
			public void handle(WorkerStateEvent event) {
				...
				txtResult.appendText(
						String.format("Lato %d: %d soluzioni (%d ms)\n", size, ris.soluzioni, ris.tempo / 1000000));
